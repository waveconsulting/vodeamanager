...
class RouteService
{
    ...
    private function createRoute($name, string $controller = '', array $options = []) {
        ...
        if (in_array('create', $only)) Route::get("/{$name}","{$controller}@create")->name($name . '.create')->middleware($middleware['create'] ?? null);
        if (in_array('show', $only)) Route::get("/{$name}/{id}","{$controller}@show")->name($name . '.show')->middleware($middleware['show'] ?? null);
        if (in_array('json', $only)) Route::get("/{$name}/json/{id}","{$controller}@json")->name($name . '.json')->middleware($middleware['json'] ?? null);
        ...
        if (in_array('edit', $only)) Route::get("/{$name}/edit","{$controller}@edit")->name($name . '.edit')->middleware($middleware['edit'] ?? null);
    }
}
